    ...
    it("Should return 200, with correct details, but no attendees", () => {
        return chai.request(SERVER_URL)
            ...
            .then((res) => {
                ...
                expect(res.body).to.have.property("name").that.is.null;
                //expect(res.body).to.have.property("description")
                expect(res.body).to.have.property("location").that.is.null;
                expect(res.body).to.have.property("start").that.is.null;
                expect(res.body).to.have.property("close_registration").that.is.null;
               ...
               // expect(res.body.close_registration).to.equal(Number(good_event_data[0].close_registration))
                expect(res.body.max_attendees).that.equals(300)
                ...
                expect(res.body.questions[1].question_id).to.equal(4)
                expect(res.body.questions[0].question_id).to.equal(3)
    ...
    it("Should return 200, with correct details, and also include the list of attendees", () => {
        return chai.request(SERVER_URL)
            ...
            .then((res) => {
                ...
                expect(res.body.attendees.length).to.equal(1)
                ...
                expect(res.body.attendees[0].user_id).to.equal(2)
                expect(res.body.attendees[0].first_name).to.equal(good_user_data[1].first_name)
                expect(res.body.attendees[0].last_name).to.equal(good_user_data[1].last_name)
                expect(res.body.attendees[0].email).to.equal(good_user_data[1].email)
